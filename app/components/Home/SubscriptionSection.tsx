    ...
    <section className="py-10 bg-white ">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <h2
            className={`${isFinnish ? "text-3xl sm:text-4xl" : "text-3xl sm:text-4xl lg:text-5xl"} font-bold text-black mb-4`}
          >
            {t("headline")}
          </h2>
          <p
            className={`${isFinnish ? "text-lg" : "text-xl"} text-gray-700 mb-6 max-w-3xl mx-auto`}
          >
            {t("subheadline")}
          </p>
          <p
            className={`${isFinnish ? "text-base" : "text-lg"} text-gray-600 leading-relaxed max-w-4xl mx-auto`}
          >
            {t("description")}
          </p>
        </div>
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`relative p-8 rounded-2xl border-2 transition-all hover:shadow-xl ${
                plan.popular
                  ? "border-[#7c9885] shadow-lg"
                  : "border-gray-200 hover:border-[#7c9885]"
              }`}
            >
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#7c9885] text-white px-4 py-1 rounded-full text-sm font-semibold">
                  Most Popular
                </div>
              )}
              ...
              <div className="text-center mb-6">
                <h3
                  className={`${isFinnish ? "text-xl" : "text-2xl"} font-bold text-black mb-3`}
                >
                  {plan.title}
                </h3>
                <p
            ...
            {benefits.map((benefit, index) => (
              <div key={index} className="flex items-start space-x-3">
                <div className="flex-shrink-0 mt-1">
                  <Check className="h-6 w-6 text-[#7c9885]" />
                </div>
